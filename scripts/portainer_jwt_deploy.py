#!/usr/bin/env python3
"""
SafeWork Portainer JWT 배포 스크립트
JWT 토큰 기반 인증으로 더 안정적인 배포
"""
...
PORTAINER_PASSWORD = os.getenv("PORTAINER_PASSWORD", "bingogo1")
...
# --- API 요청 함수들 ---
def get_jwt_token():
    """Portainer API JWT 토큰을 획득"""
    try:
        auth_url = f"{PORTAINER_URL}/api/auth"
        payload = {"username": PORTAINER_USERNAME, "password": PORTAINER_PASSWORD}
        response = requests.post(auth_url, json=payload, verify=VERIFY_SSL, timeout=10)
        response.raise_for_status()

        jwt_token = response.json()["jwt"]
        log_success("JWT 토큰 획득 성공")
        return jwt_token
    except requests.exceptions.RequestException as e:
        log_error(f"JWT 토큰 획득 실패: {e}")
        return None

def get_existing_stack(token):
    """기존 스택 조회"""
    try:
...
def create_stack(token):
    ...
    # Standalone Docker 환경은 type=2
    create_url = f"{stacks_url}?type=2&method=string&endpointId={ENDPOINT_ID}"
...
def update_stack(token, stack_id):
    """기존 스택 업데이트"""
...
def check_container_status(token):
    """컨테이너 상태 확인"""
...
def deploy_safework():
    ...
    print("\\033[0;36m")
    print("=" * 80)
    print("      SafeWork Portainer JWT 배포 시스템 v1.0.0")
    print("=" * 80)
    print("\\033[0m")

    # 1. JWT 토큰 획득
    log_info("1/5: JWT 토큰 획득...")
    jwt_token = get_jwt_token()
    if not jwt_token:
        log_error("JWT 토큰 획득 실패. 배포를 중단합니다.")
        return False

    # 2. 기존 스택 확인
    log_info("2/5: 기존 스택 확인...")
    existing_stack = get_existing_stack(jwt_token)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command == "config":
            ...
            print("사용법: python portainer_jwt_deploy.py [deploy|config]")
    else:
        deploy_safework()