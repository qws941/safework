export function getSurvey002Html(): string {
  return `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .survey-header {
            background: linear-gradient(135deg, var(--sw-warning), #e68500);
            color: white;
            padding: 40px 30px;
            text-align: center;
        }
        ...
        .section-card::before {
            ...
            right: 0;
            height: 4px;
            background: linear-gradient(90deg, var(--sw-warning), #e68500);
        }
        ...
        .section-title {
            ...
            margin-bottom: 24px;
            padding-bottom: 12px;
            border-bottom: 2px solid var(--sw-warning);
            display: flex;
            align-items: center;
            gap: 10px;
        }
...
<body>
    ...
    <script>
        ...
        // 폼 제출 처리
        document.getElementById('musculoskeletalProgramForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            for (const [key, value] of formData.entries()) {
                if (!checkboxGroups.some(group => key.includes(group))) {
                    data[key] = value;
                }
            }
            ...
            categories.forEach(cat => {
                data[\`\${cat}_total\`] = document.getElementById(\`\${cat}_total\`).textContent;
                data[\`\${cat}_risk\`] = document.getElementById(\`\${cat}_risk\`).textContent;
            });

            const surveyId = 'survey_002_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
            ...
            try {
                const response = await fetch('/api/survey/submit', {
                    ...
                    body: JSON.stringify({
                        id: surveyId,
                        form_type: '002_musculoskeletal_program',
