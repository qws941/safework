...
class DocumentUploadForm(FlaskForm):
    ...
    description = TextAreaField(
        "문서 설명",
        validators=[Optional(), Length(max=1000, message="설명은 1000자 이내여야 합니다.")],
        widget=TextArea(),
    )
    ...
    category_id = SelectField(
        "카테고리", coerce=int, validators=[DataRequired(message="카테고리를 선택해주세요.")]
    )
...
class DocumentEditForm(FlaskForm):
    ...
    title = StringField(
        "문서 제목",
        validators=[DataRequired(message="문서 제목을 입력해주세요."), Length(min=2, max=200)],
    )

    description = TextAreaField("문서 설명", validators=[Optional(), Length(max=1000)])
    ...
    version = StringField("버전", validators=[Optional(), Length(max=20)])

    change_description = TextAreaField(
        "변경 사항", validators=[Optional(), Length(max=500)]
    )
...
class CategoryForm(FlaskForm):
    ...
    name = StringField(
        "카테고리명",
        validators=[DataRequired(message="카테고리명을 입력해주세요."), Length(min=2, max=100)],
    )
    ...
    sort_order = IntegerField("정렬 순서", validators=[Optional()], default=0)
...
class DocumentTemplateForm(FlaskForm):
    ...
    name = StringField(
        "템플릿명",
        validators=[DataRequired(message="템플릿명을 입력해주세요."), Length(min=2, max=100)],
    )
    ...
    instructions = TextAreaField("작성 안내", validators=[Optional(), Length(max=1000)])
...
class BulkDocumentActionForm(FlaskForm):
    ...
    document_ids = HiddenField(
        "선택된 문서들", validators=[DataRequired(message="문서를 선택해주세요.")]
    )
    ...
    confirm = BooleanField(
        "작업을 확인합니다", validators=[DataRequired(message="작업을 확인해주세요.")]
    )
...
class DocumentVersionForm(FlaskForm):
    ...
    version_number = StringField(
        "버전 번호",
        validators=[DataRequired(message="버전 번호를 입력해주세요."), Length(min=1, max=20)],
        render_kw={"placeholder": "2.0"},
    )

    change_description = TextAreaField(
        "변경 사항 설명",
        validators=[DataRequired(message="변경 사항을 설명해주세요."), Length(min=10, max=500)],
    )
