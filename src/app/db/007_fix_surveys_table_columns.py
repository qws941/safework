    ...
    with db.engine.begin() as conn:
        ...
        for column_name, column_type in missing_columns:
            ...
            if result[0] == 0:  # Column doesn't exist
                conn.execute(text(f"ALTER TABLE surveys ADD COLUMN {column_name} {column_type}"))
...
def downgrade():
    ...
    with db.engine.begin() as conn:
        ...
        conn.execute(text("ALTER TABLE surveys DROP FOREIGN KEY IF EXISTS fk_surveys_reviewed_by"))
        
        # Note: In production, we usually don't remove columns in downgrade
        # as it could cause data loss. This is just for development.
        columns_to_remove = [
            'work_years', 'work_months', 'marriage_status', 'current_work_details',
            'current_work_years', 'current_work_months', 'break_time_minutes', 
            'break_frequency', 'previous_work_details', 'previous_work_years',
            'previous_work_months', 'hobby_computer', 'hobby_instrument',
            'hobby_knitting', 'hobby_racket_sports', 'hobby_ball_sports',
            'hobby_none', 'housework_hours', 'disease_rheumatoid', 'disease_diabetes',
            'disease_lupus', 'disease_gout', 'disease_alcoholism', 'disease_status',
            'past_accident', 'accident_hand', 'accident_arm', 'accident_shoulder',
            'accident_neck', 'accident_waist', 'accident_leg', 'physical_burden',
            'has_symptoms', 'neck_data', 'shoulder_data', 'arm_data', 'hand_data',
            'waist_data', 'leg_data', 'ip_address', 'reviewed_by', 'reviewed_at'
        ]
        
        for column_name in columns_to_remove:
            conn.execute(text(f"ALTER TABLE surveys DROP COLUMN IF EXISTS {column_name}"))
