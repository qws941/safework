...
class HealthCheckPlan(db.Model):
    ...
    type = db.Column(
        db.Enum("GENERAL", "SPECIAL", "PLACEMENT", "RETURN"), nullable=False
    )
    planned_date = db.Column(db.Date)
    target_count = db.Column(db.Integer, default=0)
    completed_count = db.Column(db.Integer, default=0)
    status = db.Column(
        db.Enum("PLANNED", "IN_PROGRESS", "COMPLETED"), default="PLANNED"
    )
    ...
    targets = relationship(
        "HealthCheckTarget", back_populates="plan", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        return {
            ...
            "completion_rate": round(
                (self.completed_count / self.target_count * 100)
...
class HealthCheckTarget(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    plan_id = db.Column(
        db.Integer, db.ForeignKey("health_check_plans.id"), nullable=False
    )
    ...
    status = db.Column(
        db.Enum("SCHEDULED", "NOTIFIED", "COMPLETED", "MISSED", "EXEMPTED"),
        default="SCHEDULED",
    )
...
class HealthCheckResult(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    target_id = db.Column(
        db.Integer, db.ForeignKey("health_check_targets.id"), nullable=False
    )
    worker_id = db.Column(db.Integer, db.ForeignKey("workers.id"), nullable=False)
...
class EnvironmentMeasurement(db.Model):
    ...
    factor_type = db.Column(
        db.Enum("DUST", "NOISE", "CHEMICAL", "ILLUMINATION", "TEMPERATURE")
    )
...
class SafeworkMsds(db.Model):
    ...
    signal_word = db.Column(
        db.Enum("DANGER", "WARNING", "CAUTION"), default="WARNING"
    )  # 신호어
    ...
    # 위험도 평가
    hazard_level = db.Column(
        db.Enum("LOW", "MEDIUM", "HIGH", "VERY_HIGH"), default="MEDIUM"
    )
    ...
    def to_dict(self):
        return {
            ...
            "annual_usage_amount": float(self.annual_usage_amount)
            if self.annual_usage_amount
            else None,
            "inventory_amount": float(self.inventory_amount)
            if self.inventory_amount
            else None,
