...
def notification_settings():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            settings.browser_enabled = data.get(
                "browser_enabled", settings.browser_enabled
            )
            settings.medication_alerts = data.get(
                "medication_alerts", settings.medication_alerts
            )
            settings.visit_reminders = data.get(
                "visit_reminders", settings.visit_reminders
            )
            settings.health_check_reminders = data.get(
                "health_check_reminders", settings.health_check_reminders
            )
            settings.alert_threshold_days = data.get(
                "alert_threshold_days", settings.alert_threshold_days
            )
...
def create_notification(
    user_id, notification_type, title, message, priority="medium", data=None
):
    """알림 생성"""
    try:
...
def get_unread_notifications():
    ...
    try:
        notifications = (
            SafeworkNotification.query.filter_by(user_id=current_user.id, is_read=False)
            .order_by(SafeworkNotification.created_at.desc())
            .limit(10)
            .all()
        )
...
def check_medication_alerts(socketio):
    ...
    try:
        ...
        # 유효기간 임박 의약품
        expiring_medications = db.session.execute(
            text(
                """
...
def check_health_check_reminders(socketio):
    ...
    try:
        # 다음 건강검진이 임박한 근로자들
        upcoming_checks = db.session.execute(
            text(
                """
            ...
            for user_id in set(users_to_notify):
                ...
                notification = create_notification(
                    ...
                    data={
                        ...
                        "check_date": str(check.next_check_date),
                    },
                )
