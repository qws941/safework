...
from models_safework import (
    Worker, Department, 
    HealthCheckPlan, HealthCheckTarget, HealthCheckResult,
    MedicalVisit, Medication,
    EnvironmentMeasurementPlan, EnvironmentMeasurement
)
    ...
    if search:
        query = query.filter(
            db.or_(
                ...
                Worker.employee_number.contains(search)
            )
        )
...
def get_worker_detail(worker_id):
    ...
    # 최근 건강검진 결과
    latest_health_check = HealthCheckResult.query.filter_by(
        worker_id=worker_id
    ).order_by(HealthCheckResult.check_date.desc()).first()
    
    # 최근 의무실 방문
    recent_visits = MedicalVisit.query.filter_by(
        worker_id=worker_id
    ).order_by(MedicalVisit.visit_date.desc()).limit(5).all()
    
    data = worker.to_dict()
    data['latest_health_check'] = latest_health_check.to_dict() if latest_health_check else None
...
def get_health_check_targets():
    ...
    for target in targets.items:
        target_dict = {
            ...
            'department': target.worker.department.name if target.worker and target.worker.department else None,
            'scheduled_date': target.scheduled_date.isoformat() if target.scheduled_date else None,
            'actual_date': target.actual_date.isoformat() if target.actual_date else None,
...
def get_medical_visits():
    ...
    if date_from:
        query = query.filter(MedicalVisit.visit_date >= datetime.strptime(date_from, '%Y-%m-%d'))
    
    if date_to:
        query = query.filter(MedicalVisit.visit_date <= datetime.strptime(date_to, '%Y-%m-%d'))
