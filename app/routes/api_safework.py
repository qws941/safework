...
from models import db, Company, Process
...
def get_companies():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "업체 목록 조회 중 오류가 발생했습니다"}), 500
...
def get_processes():
    ...
    try:
        ...
        current_app.logger.error(f"Error fetching processes: {str(e)}")
        return jsonify({"success": False, "error": "공정 목록 조회 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/companies", methods=["POST"])
@login_required
def create_company():
    """업체 등록 (관리자용)"""
    try:
        ...
        if Company.query.filter_by(name=data["name"]).first():
            return jsonify({"success": False, "error": "이미 존재하는 업체명입니다"}), 400
        ...
        return jsonify({"success": False, "error": "업체 등록 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/processes", methods=["POST"])
@login_required
def create_process():
    """공정 등록 (관리자용)"""
    try:
        ...
        # 중복 체크
        if Process.query.filter_by(name=data["name"]).first():
            return jsonify({"success": False, "error": "이미 존재하는 공정명입니다"}), 400
        ...
        db.session.rollback()
        current_app.logger.error(f"Error creating process: {str(e)}")
        return jsonify({"success": False, "error": "공정 등록 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/companies/<int:company_id>", methods=["PUT"])
@login_required
def update_company(company_id):
    ...
    try:
        ...
        company.display_order = data.get("display_order", company.display_order)
        ...
        return jsonify({"success": False, "error": "업체 정보 수정 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/processes/<int:process_id>", methods=["PUT"])
@login_required
def update_process(process_id):
    ...
    try:
        ...
        # 공정명 중복 체크 (자기 자신 제외)
        existing = Process.query.filter(Process.name == data["name"], Process.id != process_id).first()
        if existing:
            return jsonify({"success": False, "error": "이미 존재하는 공정명입니다"}), 400
        ...
        process.display_order = data.get("display_order", process.display_order)
        ...
        db.session.rollback()
        current_app.logger.error(f"Error updating process: {str(e)}")
        return jsonify({"success": False, "error": "공정 정보 수정 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/companies/<int:company_id>", methods=["DELETE"])
@login_required
def delete_company(company_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "업체가 비활성화되었습니다"})
    
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting company: {str(e)}")
        return jsonify({"success": False, "error": "업체 삭제 중 오류가 발생했습니다"}), 500


@api_safework_bp.route("/processes/<int:process_id>", methods=["DELETE"])
@login_required
def delete_process(process_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "공정이 비활성화되었습니다"})
    
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting process: {str(e)}")
        return jsonify({"success": False, "error": "공정 삭제 중 오류가 발생했습니다"}), 500
