...
def health_check_summary_report():
    ...
    try:
        ...
        # 1. 전체 통계
        total_workers_query = db.session.execute(text("""
        ...
        # 2. 건강검진 실시 현황
        health_check_query = db.session.execute(text("""
            SELECT 
        ...
        # 3. 부서별 건강검진 현황
        dept_stats_query = db.session.execute(text("""
            SELECT 
                sw.department,
        ...
        # 5. 의무실 방문 통계
        visit_stats_query = db.session.execute(text("""
            SELECT 
...
def worker_health_profile(worker_id):
    ...
    try:
        # 근로자 기본 정보
        worker_query = db.session.execute(text("""
        ...
        # 건강검진 이력
        health_checks_query = db.session.execute(text("""
        ...
        # 의무실 방문 이력
        medical_visits_query = db.session.execute(text("""
        ...
        if worker_info[4]:  # birth_date
            birth_date = worker_info[4] if isinstance(worker_info[4], date) else datetime.strptime(worker_info[4], '%Y-%m-%d').date()
            today = date.today()
...
def generate_excel_report(report_data, year):
    ...
    filename = f"SafeWork_건강검진보고서_{year}년_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def generate_medication_excel_report(medications, alerts, total_value):
    ...
    # 헤더
    headers = ['의약품명', '분류', '단위', '현재재고', '최소재고', '유효기간', 
              '공급업체', '단가', '최근입고일', '재고상태', '유효기간상태', '재고가치']
