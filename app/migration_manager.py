...
class MigrationManager:
        ...
        for filename in os.listdir(self.migrations_dir):
            if filename.endswith('.py') and filename != '__init__.py':
                try:
                    ...
                    current_app.logger.warning(f"Skipping invalid migration file: {filename}")
    ...
    def get_applied_migrations(self) -> List[str]:
        """적용된 마이그레이션 버전 목록 반환"""
        try:
            migrations = Migration.query.filter_by(success=True).order_by(Migration.version).all()
    ...
    def rollback_migration(self, filename: str) -> bool:
        ...
        try:
            ...
            if hasattr(module, 'downgrade'):
                ...
                current_app.logger.warning(f"Migration {filename} missing 'downgrade' function")
                return False
    ...
    def create_migration(self, description: str) -> str:
        ...
        if existing_files:
            last_version = max([
                int(self._parse_migration_filename(f)[0]) 
