...
Created: 2025-09-11
Description: Create MSDS tables and populate with sample data
"""
...
def upgrade():
    """Create MSDS tables and populate with sample data"""
    logger = logging.getLogger(__name__)
    logger.info("Starting MSDS tables creation...")
    
    try:
        # Create MSDS tables using raw SQL for better control
        db.engine.execute("""
        ...
        db.engine.execute("""
            CREATE TABLE IF NOT EXISTS msds_usage_records (
                ...
                quantity_unit VARCHAR(20) COMMENT '단위',
        ...
        logger.info("MSDS tables created successfully")
        
        # Check if sample data already exists
        result = db.engine.execute("SELECT COUNT(*) as count FROM msds")
        count = result.fetchone()['count']
        
        if count == 0:
            logger.info("Inserting sample MSDS data...")
            
            # Insert sample MSDS data
            db.engine.execute("""
            ...
            logger.info("Sample MSDS data inserted successfully")
        else:
            logger.info(f"MSDS data already exists ({count} records), skipping sample data insertion")
            
        db.session.commit()
        logger.info("MSDS migration completed successfully")
        return True
        
    except Exception as e:
        logger.error(f"MSDS migration failed: {e}")
        db.session.rollback()
        raise
...
def downgrade():
    """Drop MSDS tables"""
    logger = logging.getLogger(__name__)
    logger.info("Dropping MSDS tables...")
    
    try:
        db.engine.execute("DROP TABLE IF EXISTS msds_usage_records")
        db.engine.execute("DROP TABLE IF EXISTS msds_components") 
        db.engine.execute("DROP TABLE IF EXISTS msds")
        
        db.session.commit()
        logger.info("MSDS tables dropped successfully")
        return True
        
    except Exception as e:
        logger.error(f"MSDS downgrade failed: {e}")
        db.session.rollback()
        raise
