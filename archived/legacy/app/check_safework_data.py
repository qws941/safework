    ...
    with app.app_context():
        try:
            ...
            if workers_count > 0:
                result = db.session.execute(
                    text(
                        "SELECT employee_number, name, department FROM safework_workers LIMIT 3"
                    )
                )
            ...
            if meds_count > 0:
                result = db.session.execute(
                    text("SELECT name, current_stock FROM safework_medications LIMIT 3")
                )
            ...
            result = db.session.execute(
                text("SELECT COUNT(*) FROM safework_medical_visits")
            ).fetchone()
