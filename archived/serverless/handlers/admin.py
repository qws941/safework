"""
SafeWork Admin Lambda Handler
관리자 기능 관련 모든 요청을 처리하는 Lambda 함수
"""
import json
import os
import sys
...
def init_app():
    ...
    logger.info("Initializing Flask app for Admin Lambda")
    ...
    try:
        ...
        # 관리자 관련 블루프린트만 등록 (다른 블루프린트는 제거)
        blueprints_to_remove = []
        for bp_name, bp in app.blueprints.items():
            if not bp_name.startswith('admin') and bp_name not in ['main', 'migration']:
                blueprints_to_remove.append(bp_name)

        for bp_name in blueprints_to_remove:
            app.blueprints.pop(bp_name, None)

        logger.info(f"Admin Lambda initialized with blueprints: {list(app.blueprints.keys())}")
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not any(path.startswith(allowed_path) for allowed_path in allowed_paths):
        logger.warning(f"Unauthorized path for Admin Lambda: {path}")
        return {
            'statusCode': 404,
            'headers': {
            ...
            'body': json.dumps({
                'error': 'Not Found',
                'message': f'Path {path} not handled by Admin Lambda',
                'handler': 'admin'
            })
        }
    ...
    try:
        ...
        if status_code >= 400:
            logger.warning(f"Admin Lambda returned {status_code} for {path}")
        else:
            logger.info(f"Admin Lambda successful {status_code} for {path}")

        # 관리자 접근 로깅 (보안)
        if status_code == 200:
            user_ip = event.get('requestContext', {}).get('identity', {}).get('sourceIp', 'unknown')
            user_agent = event.get('headers', {}).get('User-Agent', 'unknown')
            logger.info(f"Admin access: {user_ip} | {user_agent} | {path}")

        return response

    except Exception as e:
        logger.error(f"Admin Lambda error: {str(e)}")
...
try:
    init_app()
    logger.info("Admin Lambda pre-warmed successfully")
except Exception as e:
    logger.error(f"Admin Lambda pre-warm failed: {str(e)}")